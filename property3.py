...
if __name__ == "__main__":
    ...
    #init Neural network
    if(len(sys.argv) > 1):
        networks = ['models/ACASXU_run2a_3_6_batch_2000.nnet','models/ACASXU_run2a_5_1_batch_2000.nnet']
        TIMEOUT = 21600
    ...
    for network in networks[:-3]:
        ...
        if np.min(nnet.layers[7]['conc_ub'][1:]) < nnet.layers[7]['conc_lb'][0]:
            #print("Problem Infeasible")
            print_summary(network,3,'safe',time()-instance_start)
            continue
        ...
        try:
            ...
            if(adv_found.value == 1):
                ...
                print_summary(network,3,'unsafe',time()-instance_start)
            else:
                #print("No Adv")
                results.append("Safe")
                print_summary(network,3,'safe',time()-instance_start)
            ...
            print_summary(network,3,'timeout',TIMEOUT)
