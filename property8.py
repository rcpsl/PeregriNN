...
def check_potential_CE(x):
    u = nn.evaluate(x)
    if(np.argmin(u) != 0 or  np.argmin(u) != 3):
...
def run_instance(nn, input_bounds, check_property, adv_found, target):
    ...
    if np.min(nn.layers[7]['conc_lb'][[1,2,4]]) > nn.layers[7]['conc_ub'][3]:
...
if __name__ == "__main__":
    ...
    for other_out in other_ouputs:
        if np.min(nnet.layers[7]['conc_lb'][[1,2,4]]) > nnet.layers[7]['conc_ub'][3]:
            #print("Problem Infeasible")
            continue
        ...
        try:
            ...
            if(adv_found.value == 1):
                ...
                print_summary(network,8,'unsafe',time() - start_time)
                for p in processes:
                    p.terminate()
                break
            else:
                #print("No Adv")
                results.append("Safe")
                print_summary(network,8,'safe',time()-start_time)

            
        except TimeOutException as e:
            print_summary(network,8,'timeout',TIMEOUT)
